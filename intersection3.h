...
*/
  ...
  template<class T>
	...
	*/
	...
	inline int IntersectionSegmentSphere(const Sphere3<SCALAR_TYPE>& sphere, const Segment3<SCALAR_TYPE>& segment, Point3<SCALAR_TYPE> & t0, Point3<SCALAR_TYPE> & t1)
	{
		...
		typedef typename GGL::Point3< ScalarType >	Point3t;
	...
	*/
																	...
																	std::pair< SCALAR_TYPE, SCALAR_TYPE > * res=NULL)
	{
		typedef SCALAR_TYPE														ScalarType;
		typedef typename GGL::Point3< ScalarType >		Point3t;
		typedef typename GGL::Triangle3< ScalarType > Triangle3t;
  ...
  template<class T>
	...
	if (IntersectionLinePlane(pl,l,po))
		return((po-s.P0()).Norm()<=(s.P0()-s.P1()).Norm() && (po-s.P1()).Norm()<=(s.P0()-s.P1()).Norm());
    return false;
  }
									...
									ScalarType & a ,ScalarType & b, ScalarType & dist)
{
	...
	if (!bb0.Collide(bb1))
		return false;
	if (!GGL::Intersection_Segment_Box(bb1,seg,inter))
		return false;

	//first set both directions of ray
	GGL::Ray3<ScalarType> ray;
	GGL::Point3<ScalarType> dir;
	dir=(seg.P1()-seg.P0());
	dir.Normalize();
	ray.Set(seg.P0(),dir);
							...
							Box3<ScalarType> &bbox)
{
	...
	typedef typename GGL::Segment3<ScalarType> SegmentType;
	typedef typename GGL::Point3<ScalarType> CoordType;
	SegmentType diag[4];
	...
	for (int i=0;i<3;i++)
		...
		if (GGL::Intersection<SegmentType>(pl,diag[i],intersection)) 
			return true;
	return false;
}
							   ...
							   const Point3<ScalarType> &p2)
{
	...
	if (Intersection_Segment_Box<ScalarType>(bbox,GGL::Segment3<ScalarType>(p0,p1),intersection)||
		Intersection_Segment_Box<ScalarType>(bbox,GGL::Segment3<ScalarType>(p1,p2),intersection)||
		Intersection_Segment_Box<ScalarType>(bbox,GGL::Segment3<ScalarType>(p2,p0),intersection))
		return true;
